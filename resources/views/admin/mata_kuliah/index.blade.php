{{-- filepath: resources/views/admin/mata_kuliah/index.blade.php --}}
<x-app title="Data Mata Kuliah" section_title="Data Mata Kuliah">
    @if (session('success'))
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3 sm:gap-0">
        <h2 class="text-2xl sm:text-3xl font-semibold text-gray-700">Daftar Mata Kuliah</h2>
            ...
            <table class="min-w-full table-auto">
                <thead class="text-left text-sm font-medium text-sky-700 bg-sky-100">
                    <tr>
                        ...
                        <th class="px-4 py-3 border-b border-gray-300">SKS</th>
                ...
                <tbody class="text-sm text-gray-800 divide-y divide-gray-200">
                    @forelse ($mataKuliahs as $mataKuliah)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">{{ $mataKuliah->kode }}</td>
                            <td class="px-4 py-3">{{ $mataKuliah->nama }}</td>
                            <td class="px-4 py-3">{{ $mataKuliah->sks }}</td>
                            <td class="px-4 py-3">{{ $mataKuliah->dosen->nama ?? '-' }}</td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.mata-kuliah.destroy', $mataKuliah) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
        ...
        <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
            <div class="flex flex-1 justify-between sm:hidden">
                <a href="{{ $mataKuliahs->previousPageUrl() ?? '#' }}"
                    class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 {{ $mataKuliahs->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    Previous
                </a>
                <a href="{{ $mataKuliahs->nextPageUrl() ?? '#' }}"
                    class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 {{ $mataKuliahs->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
                    Next
                </a>
            </div>
            ...
            <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Menampilkan
                        <span class="font-medium">{{ $mataKuliahs->firstItem() ?? 0 }}</span>
                        sampai
                        <span class="font-medium">{{ $mataKuliahs->lastItem() ?? 0 }}</span>
                        dari
                        <span class="font-medium">{{ $mataKuliahs->total() ?? 0 }}</span>
                        data
                    </p>
                </div>
                <div>
                    <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                        {{-- Previous --}}
                        <a href="{{ $mataKuliahs->previousPageUrl() ?? '#' }}"
                            class="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 {{ $mataKuliahs->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        ...
                        {{-- Next --}}
                        <a href="{{ $mataKuliahs->nextPageUrl() ?? '#' }}"
                            class="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 {{ $mataKuliahs->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
