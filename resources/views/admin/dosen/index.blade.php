<x-app title="Data Dosen" section_title="Data Dosen">
    @if (session('success'))
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3 sm:gap-0">
        <h2 class="text-2xl sm:text-3xl font-semibold text-gray-700">Daftar Dosen</h2>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full table-auto">
                <thead class="text-left text-sm font-medium text-sky-700 bg-sky-100">
                    <tr>
                        <th class="px-4 py-3 border-b border-gray-300">NIDN</th>
                ...
                <tbody class="text-sm text-gray-800 divide-y divide-gray-200">
                    @forelse ($dosens as $dosen)
                        <tr class="hidden sm:table-row hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">{{ $dosen->alamat }}</td>
                            <td class="px-4 py-3">{{ $dosen->email }}</td>
                            <td class="px-4 py-3">{{ $dosen->telepon }}</td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-start sm:justify-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.dosen.destroy', $dosen) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            title="Hapus Dosen"
                        ...
                        <tr class="sm:hidden">
                            <td colspan="6" class="px-4 py-4">
                                <div class="bg-white rounded-lg shadow p-4 space-y-1 border">
                                    ...
                                    <p><span class="font-semibold">Alamat:</span> {{ $dosen->alamat }}</p>
                                    <p><span class="font-semibold">Email:</span> {{ $dosen->email }}</p>
                                    <p><span class="font-semibold">Telepon:</span> {{ $dosen->telepon }}</p>
                                    <div class="flex items-center justify-start sm:justify-center space-x-2">
                                        <a href="{{ route('admin.dosen.edit', $dosen) }}"
                                            title="Edit Dosen"
                                            class="bg-yellow-400 hover:bg-yellow-500 text-gray-800 p-2 rounded inline-flex items-center no-underline">
        ...
        <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
            <div class="flex flex-1 justify-between sm:hidden">
                <a href="{{ $dosens->previousPageUrl() ?? '#' }}"
                    class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 {{ $dosens->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                    Previous
                </a>
                <a href="{{ $dosens->nextPageUrl() ?? '#' }}"
                    class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 {{ $dosens->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
                    Next
                </a>
            </div>
            ...
            <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Menampilkan
                        <span class="font-medium">{{ $dosens->firstItem() ?? 0 }}</span>
                        sampai
                        <span class="font-medium">{{ $dosens->lastItem() ?? 0 }}</span>
                        dari
                        <span class="font-medium">{{ $dosens->total() ?? 0 }}</span>
                        data
                    </p>
                </div>
                <div>
                    <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                        {{-- Previous --}}
                        <a href="{{ $dosens->previousPageUrl() ?? '#' }}"
                            class="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 {{ $dosens->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        ...
                        {{-- Next --}}
                        <a href="{{ $dosens->nextPageUrl() ?? '#' }}"
                            class="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 {{ $dosens->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed' }}">
